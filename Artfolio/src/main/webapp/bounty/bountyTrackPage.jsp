...
//==========================================================		
		...
		function loadBounty(mid) {	
				$.getJSON('trackDisplay.controller',{"mid" : mid},function(datas) {		
					...
					if ( datas.length == 0 ) {
						$('#msg').empty();
						$('#msg').append("<div><h4>您目前沒有追蹤的活動</h4></div>");
				    }
					...
					$.each(datas,function(idx, bounty) {		
						...
						$('#row'+count+' >div:last-child').html("<br><p><i class='glyphicon glyphicon-eye-open'></i>"+ bounty.b_click +"</p>"
																   ...
																   +"<a class='btn btn-mini btn-danger' style='font-size:12px'><span class='glyphicon glyphicon-heart-empty'></span>  取消追蹤</a>"
																   +"<a class='btn btn-mini btn-success' style='font-size:12px'><span class='glyphicon glyphicon-plus'></span>  參加</a>")
...
// 						}				
		...
		$('#bountyTable').on('click','div:nth-child(3) a',
				function(data) {  //刪除  編輯
					...
					var toId = $(this).parent('div').parent('div').children('div').children('div:eq(7)').text();
					var title = $(this).parent('div').parent('div').children('div').children('div:eq(8)').text();
					if("取消追蹤"===button.trim()){
							$.ajax({
