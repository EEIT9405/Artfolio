...
<div id="column1">
	...
	<div id="mailmsg1">(在 [垃圾桶] 中的郵件 30 天後會自動刪除)</div>
	<div id="mailmsg2"></div>
<div id="column2">
	<div id="mailmsg"></div>
	<table id="mailTable">
	...
	$(document).ready(function() {
		var mid = "${loginOK.mid}"
			...
			$('#mailbox').click(function () {			
				$(location).attr('href', '<c:url value="/mail/adminMailIndex.jsp" />')				
			});
			
			$('#junk').click(function () {			
				$(location).attr('href', '<c:url value="/mail/adminJunkemail.jsp" />')			
			});
			...
			$.getJSON("mailDisplay.controller",{"targetId":mid,"ShowReadOrDelete":"delete"},function(datas) {
				...
				$.each(datas, function(idx, mail) {
					var cell1 = $('<td></td>').html("<div style='display:none'>"+mail.mailid+"</div>");
					   ...
					   var cell5 = $('<td></td>').html("<input type='button' name='delete' value='復原'/>");
